...
class GrailsEntityPOJOClass extends EntityPOJOClass {
    ...
    String renderId() {
        ...
        if (c2j.isComponent(idProperty)) {
            ...
            idProperty.value.propertyIterator.each {
                idDef << delimiter << QUOTE << findRealIdName(it)[0..-3] << QUOTE
                delimiter = ', '
            }
            idDef << ']' << newline
            return idDef
    ...
    String renderHashCodeAndEquals() {
        ...
        allPropertiesIterator.each { Property property ->
            ...
            String name = findRealIdName(property)[0..-3]
    ...
    String renderConstraints() {
        ...
        allPropertiesIterator.findAll{it.name!="f"}.each { Property property ->
    ...
    String renderMany() {
        ...
        if (belongs) {
            many << combine('static belongsTo = [', ', ', ']', belongs.collect{it[0].toLowerCase()+it[1..-1]+" : "+it}, true) << newline
        }
        many
    }
        ...
        super.allPropertiesIterator.each { Property property ->
            ...
            if (property.value instanceof org.hibernate.mapping.Set) {
                ...
                if ((bidirectionalManyToOne && !isManyToMany && !isReallyManyToMany) || isManyToMany) {
                    ...
                    hasMany << property.name[0..-2] + ' : ' + classShortName
                    if (isManyToMany) {
    ...
    String renderProperties() {
        ...
        allPropertiesIterator.findAll{Property it ->it.name!="f" && (it.name.size()>2?it.name[-2..-1]:it.name)!="Id"}.each { Property property ->
    ...
    String getJavaTypeName(Property p, boolean useGenerics) {
        ...
        //        print p.getPersistentClass().getClassName()=="com.utez.Semblanza"?p.getPersistentClass().getClassName():""
        //        print p.getPersistentClass().getClassName()=="com.utez.Semblanza"?p.name:""
        //        println p.getPersistentClass().getClassName()=="com.utez.Semblanza"?p.getType().sqlTypes():""
